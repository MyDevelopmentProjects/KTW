 ...
 */
angular.module('app').controller('MobileController',
    ['$scope', '$http', 'GridManager', 'ModalManager', function ($scope, $http, GridManager, ModalManager) {

        angular.extend($scope, {
            url:'mobileUser/pagin',
            saveURL:'mobileUser/put',
            deleteURL:'mobileUser/delete',
            init:{}
        });
            ...
            $http.post($scope.saveURL, objectCopy).success(function (data) {
                if (!data.success) {
                    if(data.message === "RECORD_IS_USED_IN_OTHER_TABLES"){
                        $scope.showErrorModal("მოცემული ჩანაწერის შენახვა შეუძლებელია რადგან ის ფიქსირდება სხვა ცხრილშიც.")
                    }
                    return;
                }
